...
int main()
{
	...
	memset(&echoServAddr, 0, sizeof(echoServAddr));
  	...
  	echoServAddr.sin_addr.s_addr = htonl(INADDR_ANY);  // INADDR_ANY는 주소를 자동할당시켜 준다. 직접 입력해도 되나, 메모리에 저장되는 방식에 유의해야 한다.(Little->Big(표준)), 서버 주소는 반드시 고정되어야 하지만 클라이언트에서는 클라이언트의 주소를 자동할당하는 것이 좋다. 컴퓨터가 바뀌어도 자동 할당됨.
	...
	if(iRet < 0) {  
  	...
  	while(1)
  	{
		...
		if(iRet < 0) {
      			...
      			strcpy(m_message.m_buffer, "error: server is destroyed, good bye");
    		...
    		if(FD_ISSET(servSock, &status) == 1)  //새로운 클라이언트 접속
    		{
